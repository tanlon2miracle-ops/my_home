...
> 💡 **说明**：我目前通过 GitHub Copilot 的 API 代理使用 Claude Opus 4.6，Copilot Pro 订阅包含一定的免费额度，超出部分按 token 计费。实际花费取决于使用强度和模型选择。
